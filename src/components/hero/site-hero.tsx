...
export const SiteHero: React.FC<Props> = ({}) => {
  const [hasTriggeredHeroScroller, setHasTriggeredHeroScroller] = useState(false)
  ...
  const { ref: heroRef, inView: heroInView } = useInView({
    ...
    onChange(inView) {
      if (inView) {
        console.log('have i triggered scroll state::: ', hasTriggeredHeroScroller)
  ...
  return (
    ...
    >
      <div ref={heroRef} className="absolute inset-x-0 bottom-[4rem] h-1 w-full"></div>
      <div className="card-content gutter flex h-full w-full items-center justify-normal">
        <div className="relative mx-auto flex w-full max-w-3xl flex-col items-center justify-center text-center">
          <div ref={ref} className="absolute inset-x-0 top-[-2rem] h-2 w-full"></div>
          <Title className="font-mono text-balance font-bold text-white" level={2} showAs={3}>
          ...
          <Link href={'/profile'} className={buttonVariants({ className: 'mt-4' })}>
            Go Pro!
          </Link>
        </div>
      </div>
